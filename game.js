...
const network = {
  ws: null,
  backendUrl: 'wss://echo-orbit-backend.railway.app',
  connected: false,
  roomId: null,
  playerId: null,
};
...
function connectNetwork() {
  ...
  pushAlert('バックエンドに接続中...');
  
  try {
    network.ws = new WebSocket(network.backendUrl);

    network.ws.onopen = () => {
      network.connected = true;
      pushAlert('接続成功。マッチメイキング中...');
    ...
    network.ws.onmessage = e => {
      const data = JSON.parse(e.data);
      handleServerMessage(data);
    };

    network.ws.onerror = err => {
...
function handleServerMessage(data) {
  ...
  if (data.type === 'matched') {
    ...
    pushAlert('マッチング成功！ゲーム開始');
    status.textContent = `ルーム ${data.roomId} - 対人戦闘開始`;
  }
  ...
  if (data.type === 'player_update') {
    if (data.playerId !== network.playerId) {
      ...
      remotePlayers[data.playerId].x = data.x;
      remotePlayers[data.playerId].y = data.y;
      remotePlayers[data.playerId].heading = data.heading;
      remotePlayers[data.playerId].moving = data.moving;
      remotePlayers[data.playerId].pulse = data.pulse;

      if (distance(player, remotePlayers[data.playerId]) < 140 && data.moving) {
        remotePlayers[data.playerId].heard = true;
  ...
  if (data.type === 'event') {
    if (data.event === 'pulse') {
      if (remotePlayers[data.playerId]) {
        echoes.push({
          x: remotePlayers[data.playerId].x,
          y: remotePlayers[data.playerId].y,
          radius: 0,
          max: 420,
          alpha: 0.3,
          color: 'cyan',
  ...
  if (data.type === 'player_left') {
    ...
    pushAlert('プレイヤーが退出しました');
  }

  if (data.type === 'player_defeated') {
    pushAlert(`${data.playerId} が倒されました`);
  }
}

function broadcastNetwork(message) {
  if (!network.connected || !game.inRoom) return;
...
function gameLoop() {
  if (game.alive) {
    ...
    updatePulse();
    updateEchoes();
    if (game.inRoom && network.connected) {
      networkSendTimer -= 1;
